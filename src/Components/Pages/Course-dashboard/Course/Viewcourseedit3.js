...
const EditCourse = () => {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        ...
        console.log("📖 Sample lesson names:", course.chapters?.[0]?.lessons?.map(l => l.lessonname));
            ...
            lessons: (chapter.lessons || []).map((lesson) => {
              ...
              if (typeof parsedLesson.audioFile === 'string' && parsedLesson.audioFile) {
                try {
                  ...
                  console.log(`✅ Parsed audioFile for "${lesson.lessonname}":`, parsedLesson.audioFile);
                } catch (e) {
                  console.warn(`❌ Failed to parse audioFile for "${lesson.lessonname}":`, e);
              ...
              if (typeof parsedLesson.videoFile === 'string' && parsedLesson.videoFile) {
                try {
                  ...
                  console.log(`✅ Parsed videoFile for "${lesson.lessonname}":`, parsedLesson.videoFile);
                } catch (e) {
                  console.warn(`❌ Failed to parse videoFile for "${lesson.lessonname}":`, e);
              ...
              if (typeof parsedLesson.pdfFile === 'string' && parsedLesson.pdfFile) {
                try {
                  ...
                  console.log(`✅ Parsed pdfFile for "${lesson.lessonname}":`, parsedLesson.pdfFile);
                } catch (e) {
                  console.warn(`❌ Failed to parse pdfFile for "${lesson.lessonname}":`, e);
        ...
        console.log("✅ Course data loaded with calculated breakdown:", calculatedBreakdown);
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        try {
          ...
          for (const urlData of presignedUrls) {
            ...
            if (file) {
              ...
              console.log(`📤 Uploading ${file.name} to S3 (${uploadedCount}/${totalFiles})...`);
          ...
          showMessage("success", `✅ All ${totalFiles} files uploaded to S3 successfully!`);
        } catch (s3Error) {
      ...
      // =====================================================
      console.log("📋 Preparing course update data...");
      ...
      console.log("📤 Sending course update (metadata + S3 URLs only)...");
                        ...
                        <Grid container spacing={1.5}>
                          <Grid item xs={6}>
                            ...
                            <Typography variant="body2" sx={{ fontWeight: 600, color: "#111827" }}>
                              ₹{formData.price.breakdown.courseValue?.toFixed(2) || "0.00"}
                            </Typography>
                          </Grid>
                          <Grid item xs={6}>
                            <Typography variant="caption" sx={{ color: "#6b7280", display: "block" }}>
                            ...
                            <Typography variant="body2" sx={{ fontWeight: 600, color: "#111827" }}>
                              ₹{formData.price.breakdown.gst?.cgst?.toFixed(2) || "0.00"}
                            </Typography>
                          </Grid>
                          <Grid item xs={6}>
                            <Typography variant="caption" sx={{ color: "#6b7280", display: "block" }}>
                            ...
                            <Typography variant="body2" sx={{ fontWeight: 600, color: "#111827" }}>
                              ₹{formData.price.breakdown.gst?.sgst?.toFixed(2) || "0.00"}
                            </Typography>
                          </Grid>
                          <Grid item xs={6}>
                            <Typography variant="caption" sx={{ color: "#6b7280", display: "block" }}>
                            ...
                            <Typography variant="body2" sx={{ fontWeight: 600, color: "#111827" }}>
                              ₹{formData.price.breakdown.transactionFee?.toFixed(2) || "0.00"}
                            </Typography>
                          </Grid>
                          <Grid item xs={12}>
                            <Divider sx={{ my: 1 }} />
                            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                              ...
                              <Typography variant="body1" sx={{ fontWeight: 700, color: "#6366f1" }}>
                                ₹{formData.price.breakdown.gst?.total?.toFixed(2) || "0.00"}
                              </Typography>
                            </Box>
                          </Grid>
                        </Grid>
