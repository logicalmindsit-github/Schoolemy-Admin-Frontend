...
const CreateCourses = () => {
  ...
  useEffect(() => {
    ...
    // finalPrice = courseValue × 1.20 (includes 18% GST + 2% transaction fee)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (filesToUpload.length > 0) {
        ...
        console.log(`✅ Received ${urlResponse.data.data.length} presigned URLs`);
        ...
        const uploadPromises = urlResponse.data.data.map(async (urlData, index) => {
          ...
          try {
            ...
            console.error(`❌ Failed to upload ${fileInfo.fileName}:`, uploadErr);
